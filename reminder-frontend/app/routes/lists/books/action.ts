...
export default async function action({ request }: ActionFunctionArgs) {
  ...
  if (intent === "update") {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_URL}/api/books/update`, {
      ...
      body: JSON.stringify({ book_id, name, author, category, notes }),
    });

    if (res.status === 401) {
      throw redirect("/login?redirectTo=/books");
    }

    if (!res.ok) {
      const text = await res.text().catch(() => "");
      throw new Response(text || `Failed to update book`, { status: res.status });
    }
